...
public final class Benchmarking
{
    public static void benchmark() {
        ...
        for (int i = 0; i < resultadosBubble.length; i++) {
            ...
            tabela.anotarAmostra(
                resultadosBubble[i].qtdeElems,
                resultadosBubble[i].tempo,
                resultadosSelection[i].tempo,
                resultadosInsertion[i].tempo);
        }
        tabela.setTitulo("Benchmarking com média de 10 amostrag");
        tabela.setLegendas("bubblesort(ms)","selectionsort(ms)","insertionsort(ms)");
        tabela.setEtiquetaX("Qnt de elementos");
    ...
    private static Medicao[] benchmarkOrdenacao(
        ...
        for (int qtde = de, r = 0; qtde < ate; qtde += passo, r++) {            
            Integer[] arrDecrescente = UteisArranjo.novoArranjoAleatorio(qtde);
